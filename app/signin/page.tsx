...
import { getT, detectLang, type Lang } from "@/lib/i18n";
    ...
    >
      ...
      >
        ...
        <div className="py-16 sm:py-0 sm:flex-1 sm:flex sm:flex-col sm:justify-center">
          ...
          >
            {t("signin_headline_line1")}{" "}
            {t("signin_headline_line2")}{" "}
            <em
              className="not-italic"
              style={{ color: "var(--accent)", fontFamily: "var(--serif)" }}
            >
              {t("signin_headline_em")}
            </em>
            .
          </h1>
        ...
        <div>
          ...
          <div className="flex items-end gap-8 flex-wrap">
            <div>
              <p className="text-[18px] font-semibold leading-none mb-1" style={{ color: "var(--ink)", fontFamily: "var(--mono)" }}>
                240M+
              </p>
              <p className="text-[10px] uppercase tracking-[1.2px]" style={{ color: "var(--ink-dim)", fontFamily: "var(--mono)" }}>
                {t("stat_papers")}
              </p>
            </div>
            <div>
              <p className="text-[15px] font-semibold leading-none mb-1" style={{ color: "var(--ink)", fontFamily: "var(--sans)" }}>
                EN · 中 · 日 · 한
              </p>
              <p className="text-[10px] uppercase tracking-[1.2px]" style={{ color: "var(--ink-dim)", fontFamily: "var(--mono)" }}>
                {t("stat_languages")}
      ...
      >
        <div className="w-full max-w-[380px] mx-auto">
          ...
          {/* error banner */}
          {errorMsg && (
            <div className="mb-4 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2.5 text-[12px] leading-relaxed" style={{ color: "var(--ink-dim)" }}>
              <span style={{ color: "#f87171" }}>{errorMsg}</span>
              {errorCode === "EmailNotFound" && (
                <> <Link href={registerHref} className="underline underline-offset-2 hover:opacity-80">
                  {t("create_account")}
                </Link></>
              )}
            </div>
          ...
          >
            <div className="flex items-center gap-3">
              <GoogleLogo />
              <span style={{ fontFamily: "var(--sans)" }}>{t("sign_in_google")}</span>
            </div>
          ...
          <div className="flex items-center gap-3 my-4">
            ...
            <span className="text-[10px] uppercase tracking-widest" style={{ color: "var(--ink-dim)", fontFamily: "var(--mono)" }}>
              {t("or")}
            </span>
            <div className="h-px flex-1" style={{ background: "var(--rule-soft)" }} />
          </div>
          ...
          <form onSubmit={handleCredentials} className="flex flex-col gap-3">
            <div>
              ...
              <input
                ...
                value={email}
                onChange={(e) => { setEmail(e.target.value); setErrorCode(""); }}
          ...
          <Link
            href={callbackUrl}
            className="flex w-full items-center justify-between rounded-lg border px-4 py-3 transition-all active:scale-[0.99]"
            style={{
              borderColor: "var(--rule)",
              color: "var(--ink)",
            }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = "var(--ink-dim)"; }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = "var(--rule)"; }}
          >
            <span className="text-[13px] italic" style={{ fontFamily: "var(--serif)", color: "var(--ink)" }}>
              {t("continue_guest")}
            </span>
            <span
              className="text-[9px] uppercase tracking-[1.5px] px-2 py-0.5 rounded"
              style={{
                fontFamily: "var(--mono)",
                color: "var(--ink-dim)",
          ...
          {/* Terms */}
          <p className="mt-5 text-center text-[11px] leading-relaxed" style={{ color: "var(--ink-dim)", fontFamily: "var(--sans)" }}>
            {t("signin_terms")}{" "}
            <Link href="/terms" className="underline underline-offset-2 transition-opacity hover:opacity-70">
              {t("terms")}
            </Link>
            {" "}{t("and")}{" "}
            <Link href="/privacy" className="underline underline-offset-2 transition-opacity hover:opacity-70">
              {t("privacy_policy")}
            </Link>
            .
          </p>
